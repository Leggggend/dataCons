...
//判断二叉树T是否为二叉排序树
{
    ...
    static KeyType preKey = -1; // 假设键值不会小于-1
    static int firstVisit = 1; // 标记是否是第一次访问
    ...
    // 递归判断左子树
    if (JudgeBiTree(T->lchild) == NO) {
        return NO; // 左子树不是BST
    }

    // 检查当前节点
    if (firstVisit) {
        firstVisit = 0; // 第一次访问时不进行比较
    } else if (T->data.key <= preKey) {
        return NO; // 当前节点的键值小于等于前一个节点的键值，不是BST
    }
    preKey = T->data.key; // 更新前一个节点的键值
